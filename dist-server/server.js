...
app.get('/api/bot', async (req, res) => {
    try {
        ...
        invoicesRaw.forEach((raw) => {
            ...
            if (clientDb?.frecuencia === 'SEMANAL')
                defaultThreshold = 7;
            else if (clientDb?.frecuencia === 'MENSUAL')
                defaultThreshold = 30;
            ...
            if (daysOverdue > clientAgg.maxDaysOverdue)
                clientAgg.maxDaysOverdue = daysOverdue;
        ...
        vendorsMap.forEach((clientsMap, vendorName) => {
            ...
            Array.from(clientsMap.values()).forEach((c) => {
            ...
            locationsMap.forEach((clientes, nombre_localidad) => {
                localidades.push({
                    ...
                    clientes: clientes.sort((a, b) => b.saldo_con_intereses - a.saldo_con_intereses)
                });
            });
