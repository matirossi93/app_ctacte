...
vi.mock('./infomanager.js', () => ({
  ...
  leerComprobante: async()=>({cabecera:{tipo_comprobante:'PR',cod_empresa:1,cod_cliente:727,existe:true,anulada:false},items:await im.getItemsComprobante()}),
  ...
  presupuestoFacturado: vi.fn(async()=>({facturado:false})),
...
function fakeSb() {
  im.sbMock.mockImplementation(() => ({
    ...
    from: (t: string) => {
      let error:any=null;let sinFila=false;
      const q: any = {
        then: (r: any, j: any) => Promise.resolve({ data: null, error }).then(r, j),
        maybeSingle: () => Promise.resolve({ data: t==='pedidos_vendedor' && !sinFila ? clonar() : null, error }),
        update: (v: any) => { updates.push([t, v]);if(fallaLocal==='final' && v.estado==='enviado')sinFila=true; return q; },
        insert: (v: any) => { insertados.push([t, v]);if(fallaLocal==='insert')error={message:'insert falló'}; return q; },
        delete: () => { borrados.push(t);if(fallaLocal==='delete')error={message:'delete falló'}; return q; },
      };
...
const COLGADO = {
  id: 'ped-1', cod_empresa:1, estado: 'sin_respuesta', cod_cliente: 727, cliente_nombre: 'PARRILLA',
...
beforeEach(() => {
  fallaLocal=''; updates = []; borrados = []; insertados = [];
...
describe('reconciliarSinRespuesta — el pedido colgado por timeout de IM', () => {
  it('🔴 SÍ ENTRÓ: adopta el presupuesto nuevo y anula el viejo', async () => {
    ...
    im.cabeceraComprobante.mockResolvedValue({ tipo_comprobante:'PR',cod_empresa:1,cod_cliente:727, fecha: '2026-09-02', anulada: false, existe: true });
  ...
  it('🔴 no encontrar el nuevo no prueba no emisión: conserva bloqueo', async () => {
  ...
  it('🔴 el viejo está anulado o borrado y el nuevo no aparece: no se toca (hace falta un humano)', async () => {
    im.buscarPresupuestoPorCompatibilidad.mockResolvedValue({ busquedaOk: true, encontrado: null });
    im.cabeceraComprobante.mockResolvedValue({ tipo_comprobante:'PR',cod_empresa:1,cod_cliente:727, fecha: null, anulada: null, existe: false });
...
it.each(['delete','insert','final','sin_items'])('conciliación %s fallida conserva locks incluso con PR viejo anulado',async fallo=>{
 fallaLocal=fallo;
 im.buscarPresupuestoPorCompatibilidad.mockResolvedValue({busquedaOk:true,encontrado:{id:'58706797',numero:58068}});
 im.cabeceraComprobante.mockResolvedValue({tipo_comprobante:'PR',cod_empresa:1,cod_cliente:727,fecha:'2026-09-02',existe:true,anulada:true});
 if(fallo==='sin_items')im.getItemsComprobante.mockResolvedValue([]);
 const p=clonar();await reconciliarSinRespuesta([p]);
 expect(p.estado).toBe('sin_respuesta');
 expect(updates.filter(([,v])=>v.im_error?.includes('Conciliación pendiente'))).toHaveLength(1);
 const calls=im.sbMock.mock.results.flatMap((r:any)=>r.value.rpc.mock.calls);
 expect(calls.some((c:any[])=>c[0]==='soltar_presupuesto')).toBe(false);
});
