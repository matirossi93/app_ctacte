 ...
 * diferencia de importe — no queda un peso sin explicar:
 *
 *   · CANTIDAD: `(cantidad_nueva − cantidad_vieja) × precio_viejo`
 *     Un producto que se saca, o del que se lleva menos, cae acá al precio al que se facturó.
 *   · PRECIO:   `cantidad_nueva × (precio_nuevo − precio_viejo)`
 *     Es lo que pasa cuando se cargó mal la lista: la misma mercadería a otro precio.
 *
 *   (q1−q0)·p0 + q1·(p1−p0) = q1·p1 − q0·p0 ✅
 *
...
export function calcularCorreccion(
  ...
  const viejos = new Map<number, RenglonCorreccion>();
  for (const r of originales) viejos.set(Number(r.cod_articulo), r);
  const nuevos = new Map<number, RenglonCorreccion>();
  for (const r of finales) nuevos.set(Number(r.cod_articulo), r);

  const agregar = (destino: RenglonCorreccion[], base: RenglonCorreccion, cantidad: number, precio: number) => {
    if (cantidad <= CERO || Math.abs(precio) <= CERO) return;
    destino.push({
  ...
  for (const cod of codigos) {
    ...
    const q0 = Number(v?.cantidad ?? 0), p0 = Number(v?.precio ?? 0);
    const q1 = Number(n?.cantidad ?? 0), p1 = Number(n?.precio ?? p0);
    const base = n ?? v!;

    // 1) La cantidad, al precio ORIGINAL: es la mercadería que se saca o se agrega.
    const dq = q1 - q0;
    if (dq < -CERO) agregar(nc, v!, -dq, p0);
    else if (dq > CERO) agregar(nd, base, dq, n ? p1 : p0);
    ...
    const dp = p1 - p0;
    if (q1 > CERO && v && Math.abs(dp) > CERO) {
      if (dp < 0) agregar(nc, v, q1, -dp);
      else agregar(nd, base, q1, dp);
    }
  }

  const suma = (rs: RenglonCorreccion[]) => centavos(rs.reduce((s, r) => s + r.cantidad * r.precio, 0));
...
export async function verFacturaParaCorregir(req: Request & { user?: JwtPayload }, res: Response) {
  ...
  try {
     ...
     * 09/09/2026.
     */
    const renglones = items.map((it: any) => ({
