...
export const Dashboard = ({ onUnauthorized }: Props) => {
    const [interestRate, setInterestRate] = useState(() => {
        const saved = localStorage.getItem('interest_rate');
    ...
    useEffect(() => {
        localStorage.setItem('interest_rate', String(interestRate));
    }, [interestRate]);
    ...
    const exportToCSV = () => {
        const headers = ['Cliente', 'Código', 'Localidad', 'Vendedor', 'Saldo', 'Con Interés', 'Días Mora'];
        ...
        const vendorSlug = currentViewName.replace(/[^a-zA-Z0-9]/g, '_').slice(0, 30);
        a.download = `cobranzas_${vendorSlug}_${date}.csv`;
        a.click();
