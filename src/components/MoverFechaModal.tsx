...
export function MoverFechaModal(
  { idFactura, onCerrar, onListo }: { idFactura: string; onCerrar: () => void; onListo: () => void },
    ...
    const operacionGlobal = useOperacionReparto('MoverFechaModal');
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      try {
        ...
        if (!vivo) return;
        setFactura(d.factura); setRequiereVerificar(false);
  ...
  async function guardar() {
    if (!cambió || requiereVerificar || !operacionGlobal.comenzar()) return;
    ...
    try {
      ...
      setError(`${e?.message ?? 'Error de conexión'}. Volvé a leer la factura antes de otro intento; el remito puede requerir revisión.`);
    } finally {
      setGuardando(false); operacionGlobal.terminar();
    }
  }

  const cerrar = () => { if (!operacionGlobal.enCurso.current) onCerrar(); };
  const dialogo = useDialogoReparto(cerrar);
  return (
    <dialog ref={dialogo} style={estiloDialogo} aria-label="Mover fecha de factura" className="mf-fondo" onClick={e => { if (e.target === e.currentTarget) cerrar(); }}>
      <div className="mf-modal">
        <header className="mf-header">
          <h3><CalendarDays size={16} /> Fecha de la factura {factura?.letra ?? ''} {factura?.numero ?? ''}</h3>
          <button onClick={cerrar} disabled={guardando} aria-label="Cerrar cambio de fecha"><X size={18} /></button>
        </header>
        ...
        {requiereVerificar && <button className="mf-btn" disabled={cargando} onClick={() => setRevision(v => v + 1)}>Volver a leer estado actual</button>}
        {listo ? (
          ...
          <>
            ...
            <label className="mf-campo">
              ...
              <input disabled={guardando} type="date" value={fecha} onChange={e => setFecha(e.target.value)} />
            </label>
            {/* 🪤 La hoja de ruta se arma con remitos: separarlos es el problema, no la solución. */}
            <label className="mf-check">
              <input disabled={guardando} type="checkbox" checked={moverRemito} onChange={e => setMoverRemito(e.target.checked)} />
              <span>Mover también el remito (la hoja de ruta se arma con el remito)</span>
            </label>
            <div className="mf-pie">
              <button className="mf-btn" onClick={cerrar} disabled={guardando} aria-label="Cerrar cambio de fecha">Cancelar</button>
              <button className="mf-btn primario" onClick={() => void guardar()} disabled={!cambió || guardando || requiereVerificar}>
